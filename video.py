...
pygame.init()
# highest value of level is 9 and upgrade is 5 till now ; update it after making changes
level = 0
disw = 1200
dish = 600
upgrade = 0
...
click1 = mixer.Sound('cannonball-89596.mp3')
screen1 = pygame.display.set_mode((1200,700))
background10 = pygame.image.load('main castle1.jpg').convert_alpha()
background = pygame.transform.scale(background10,(1200,700))
background1 = pygame.image.load('main game2.png').convert_alpha()
background2 = pygame.transform.scale(background1, (1200, 700))
background3 = pygame.image.load('vintage-grunge-paper-background.jpg').convert_alpha()
background4 = pygame.transform.scale(background3, (350, 400))
background5 = pygame.image.load('guide.png').convert_alpha()
background6 = pygame.image.load('level map2.jpg').convert_alpha()
background7 = pygame.transform.scale(background6, (1200, 700))
background8 = pygame.image.load('reset.png').convert_alpha()
background9 = pygame.transform.scale(background8,(1200,700))
background10 = pygame.image.load('level map211.png').convert_alpha()
background11 = pygame.transform.scale(background10,(1200,700))
guide = pygame.transform.scale(background5, (400, 400))

new_game = pygame.image.load('new game.png').convert_alpha()
new_game1 = pygame.transform.scale(new_game, (300, 108))
home = pygame.image.load('home.png').convert_alpha()
# home1=pygame.transform.scale(home,(20,20))
ok=pygame.image.load('ok.png').convert_alpha()
ok1=pygame.transform.scale(ok,(300,108))
exit11 = pygame.image.load('exit button.png').convert_alpha()
# exit12=pygame.transform.scale(exit11,(250,60))
resume = pygame.image.load('resume.png').convert_alpha()
# resume1=pygame.transform.scale(resume,(250,60))
help1 = pygame.image.load('help.png').convert_alpha()
# help2=pygame.transform.scale(help1,(250,60))
level1 = pygame.image.load('level1 duplicate.png').convert_alpha()
level1_dup = pygame.transform.scale(level1, (160, 160))
level2 = pygame.image.load('level2 duplicate.png').convert_alpha()
level2_dup = pygame.transform.scale(level2, (170, 170))
retry = pygame.image.load('retry button.png').convert_alpha()
level_image2 = pygame.image.load('level_image.png')  # 902,442
level_image = pygame.transform.scale(level_image2, (180, 180))
setting_image = pygame.image.load('settings.png').convert_alpha()
pygame.display.set_caption("Quest Of Glory")
icon = pygame.image.load('icon main.png')
...
start_img = pygame.image.load('strat button2.jpg').convert_alpha()
exit_img1 = pygame.image.load('quitnutton.png').convert_alpha()
exit_img = pygame.transform.scale(exit_img1, (300, 108))


#def showtext(x, y):
 #   score = font.render("QUEST OF GLORY", True, (255, 255, 255))
...
class Button():
    ...
    def draw(self):
        ...
        if self.rect.collidepoint(pos):
            if pygame.mouse.get_pressed()[0] == 1 and self.clicked <= 20:
                self.clicked += 1
                action = True
...
# video
video = moviepy.editor.VideoFileClip("hack main1.mp4")

video_width, video_height = video.size
scale_factor = max(disw / video_width, dish / video_height)
...
start_button = Button(400, 500, start_img, 0.5)
start_button2 = Button(400, 500, start_img, 0.5)

new_game_button = Button(520, 570, new_game1, 0.5)

exit_button = Button(650, 500, exit_img, 0.5)

level_button = Button(904, 360, level_image, 1)
level1_button = Button(896, 360, level1_dup, 1)
level11_button = Button(896, 360, level1_dup, 1)
level2_button = Button(710, 360, level2_dup, 1)

setting_button = Button(10, 10, setting_image, 1)
setting_button2 = Button(10, 10, setting_image, 1)

home_button = Button(60, 60, home, 1)

resume_button = Button(60, 180, resume, 1)
resume_button2 = Button(270, 320, resume, 1)

help_button = Button(60, 240, help1, 1)

exit_button1 = Button(60, 120, exit11, 1)
exit_button2 = Button(1100, 20, exit11, 1)

retry_button = Button(1100, 20, retry, 1)

ok_button= Button(520,500,ok1,.5)
...
def show_text(x, y):
    score = font.render("Warrior Health", True, (0, 0, 0))
...
while run:
    f = open("savetime1.py")
    levelrepeat = int(f.read(1))
    upgraderrepeat = int(f.read(1))
    checker = int(f.read(1))
    # print(levelrepeat,upgraderrepeat)
    if level == 0:
        ...
        if start_button.draw():
            click.play()
            print('start')
            if upgrade == 0 and checker == 0:
                level = 7
            elif (levelrepeat == 8) and (upgraderrepeat == 4):
                level = 8
            else:
                level = 1
        if exit_button.draw():
    ...
    if level == 7:
        frame1 = video.get_frame(pygame.time.get_ticks() / 1000)  # Get the current frame of the video
        frame1_surface = pygame.surfarray.make_surface(frame1)
        frame1_surface = pygame.transform.scale(frame1_surface, (disw, dish))  # Resize the frame to fit the window
        screen1.blit(frame1_surface, (0, 0))
        video.preview()
        upgrade = 1
        level = 1
    ...
    if level == 1:
        ...
        # screen.blit(leve_image,(902,442))
    ...
    if level == 2 :
        ...
        c2 = 0
        for i in range(No_of_enemy_1):
            if enemy_1_y_axis[i] <= 275 and enemy_1_y_axis[i] >= 0 and enemy_1_x_axis[i] >= 370 and enemy_1_x_axis[
                i] < 440:
                c2 += 1
        ...
        c5 = 0
        for i in range(No_of_enemy_3):
            if enemy_3_y_axis[i] <= 275 and enemy_3_y_axis[i] >= 0 and enemy_3_x_axis[i] >= 370 and enemy_3_x_axis[
                i] < 440:
                c5 += 1
        c6 = 0
        for i in range(No_of_enemy_3):
    ...
    if level == 5:
        screen1.blit(guide, (0, 0))
        # screen.blit(leve_image,(902,442))
        if resume_button2.draw():
            click.play()
            print("help quit")
            # change it after words
            level = 1
    ...
    if level == 10 :
        screen.fill(bg)
        screen.blit(back, (0, 0))
        towerhp = cannon_hp + cannon2_hp + cannon3_hp-100
        healthbar.hp = towerhp
        healthbar.draw(screen)
        if towerhp <= 0:
            dead = 1
